...
- **Course history & plan controls**: `GET /api/lessons` lists completed plans (`LessonSummary`); the progress screen gains a "Course so far" card (numbered lessons, focus tags, Goal met / Carried over per lesson) and `SessionSummary.lesson` marks plan-linked sessions with a "Lesson" badge in Recent sessions (`listSessions` uses `countDistinct` over the double join). `POST /api/plan/regenerate` drops the pending plan and generates a fresh one ("New plan" button on the lesson card — useful when the small model's plan misses); `GET /api/plan` also returns `lessonNumber` (completed plans + 1) shown as "Lesson N" on the lesson home.
