...
export function WarmupCard({ phrases, used, onPlay, lesson }: WarmupCardProps) {
  const usedCount = used.filter(Boolean).length
  const allUsed = phrases.length > 0 && usedCount === phrases.length
  ...
  return (
    <section className="animate-message-in rounded-3xl bg-card p-4 shadow-xs ring-1 ring-primary/20">
      ...
      {lesson && lesson.focusLabels.length > 0 && (
        <div className="mt-2.5 flex flex-wrap gap-1.5">
          {lesson.focusLabels.map((label) => (
            <span
              key={label}
